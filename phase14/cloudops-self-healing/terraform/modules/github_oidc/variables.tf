...
variable "launch_template_arn" {
  description = "Launch Template ARN (from the compute module) — scopes what this role can modify."
  type        = string
}

variable "asg_arn" {
  description = "Auto Scaling Group ARN (from the autoscaling module) — scopes what this role can refresh."
  type        = string
}
