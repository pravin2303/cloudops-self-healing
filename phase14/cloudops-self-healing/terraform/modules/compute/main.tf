...
resource "aws_launch_template" "app" {
  ...
  lifecycle {
    ...
    # After initial creation, user_data is intentionally owned by the
    # CD pipeline (Phase 12), which patches the image tag directly via
    # the AWS CLI without running Terraform. Without ignore_changes
    # here, the next `terraform apply` would see that as drift and
    # silently revert every CD-deployed version back to whatever
    # app_image_tag was last set in terraform.tfvars.
