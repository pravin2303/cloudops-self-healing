...
variable "force_delete" {
  description = <<-EOT
    Allows Terraform to delete the ECR repository even if it still
    contains images. Appropriate for a dev/learning environment where
    you genuinely want `terraform destroy` to fully tear down the
    project in one command. In a real production environment, this
    should almost always be false — you do NOT want a routine
    infrastructure change to be capable of silently deleting a
    repository full of production images.
  EOT
  type        = bool
  default     = true
}
